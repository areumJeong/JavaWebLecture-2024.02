...
public class UserController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "register":
			if (method.equals("GET")) {
				session.invalidate();
//				rd = request.getRequestDispatcher("/bbs/user/register.jsp");
				...
				if (uSvc.getUserByUid(uid) != null) {
					rd = request.getRequestDispatcher("/bbs/user/alertMsg.jsp");
					request.setAttribute("msg", "아이디가 중복입니다.");
					request.setAttribute("url", "/WEB-INF/view/user/register");
					rd.forward(request, response); 
		...
		case "update":
			if (method.equals("GET")) {
				uid = request.getParameter("uid");
				user = uSvc.getUserByUid(uid);
//				rd = request.getRequestDispatcher("/bbs/user/update.jsp");
				rd = request.getRequestDispatcher("/WEB-INF/view/update.jsp");
